...
export const searchCodeSchema = z.object({
  ...
  file_pattern: z.string().optional().describe("File glob, e.g. '*.tsx', '*.ts'. Default: '*.{ts,tsx}'"),
...
export async function searchCode(input: SearchCodeInput): Promise<string> {
  const results = grepRepo(input.query, {
    glob: input.file_pattern || "*.{ts,tsx}",
  ...
  if (!results.trim()) {
    return `No matches found for pattern: "${input.query}" in ${input.file_pattern || "*.{ts,tsx}"} files.`;
  }

  const root = getRepoRoot();
  const processed = results.split("\n")
    .map((line) => line.startsWith(root) ? line.substring(root.length + 1) : line)
    .join("\n");
