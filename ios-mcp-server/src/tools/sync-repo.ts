/**
 * Tool: sync_repo
 * Pull latest changes from the iOS repo's dev branch.
 * Also shows current sync status (last sync time, current HEAD).
 */
import { z } from "zod";
import {
  syncRepo,
  getSyncStatus,
  GitCacheConfig,
} from "../git-cache.js";
import { getRepoRoot } from "../repo.js";

export const syncRepoSchema = z.object({
  action: z
    .enum(["sync", "status"])
    .optional()
    .default("sync")
    .describe(
      "'sync' pulls latest changes from dev branch. 'status' shows current cache state without fetching."
    ),
});

export type SyncRepoInput = z.infer<typeof syncRepoSchema>;

// The config is injected at registration time from index.ts
let _config: GitCacheConfig | null = null;

export function setSyncConfig(config: GitCacheConfig) {
  _config = config;
}

export async function syncRepoTool(input: SyncRepoInput): Promise<string> {
  const cacheDir = getRepoRoot();

  if (input.action === "status") {
    const status = getSyncStatus(cacheDir);
    if (!status.exists) {
      return "No cached repo found. The repo will be cloned on first tool call.";
    }
    const lines = [
      `# Repo Cache Status`,
      `  Path: ${cacheDir}`,
      `  Branch: ${status.branch || "unknown"}`,
      `  HEAD: ${status.head || "unknown"}`,
      `  Last synced: ${status.lastSync || "unknown"}`,
    ];
    return lines.join("\n");
  }

  // Sync
  if (!_config) {
    return "Error: Repo URL not configured. The server must be started with --repo-url.";
  }

  const result = await syncRepo(_config);
  const lines = [
    `# Repo Sync Result`,
    ``,
    result.summary,
    ``,
    `  Branch: ${_config.branch}`,
    `  Cache: ${_config.cacheDir}`,
    `  Last synced: ${new Date().toISOString()}`,
  ];
  return lines.join("\n");
}
